...
//! ## Overview
//! Concurrency and ordering-insensitive determinism coverage.
//! Purpose: Ensure deterministic outcomes/runpacks under concurrent runs.
//! Invariants:
...
) -> decision_gate_core::ScenarioSpec {
...
) -> Result<ScenarioDefineResponse, Box<dyn std::error::Error>> {
...
async fn export_runpack_to_dir(
    client: &helpers::mcp_client::McpHttpClient,
    scenario_id: &decision_gate_core::ScenarioId,
    tenant_id: decision_gate_core::TenantId,
    namespace_id: decision_gate_core::NamespaceId,
...
fn verify_trigger_decision_records(runpack_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
...
async fn metamorphic_evidence_order_canonical_in_runpack() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let namespace_id = decision_gate_core::NamespaceId::from_raw(1)
        .ok_or_else(|| std::io::Error::other("nonzero namespace id"))?;
    let tenant_id = decision_gate_core::TenantId::from_raw(1)
        .ok_or_else(|| std::io::Error::other("nonzero tenant id"))?;
    let run_id = decision_gate_core::RunId::new("run-1");
    let spec = ordered_evidence_spec(scenario_id.clone(), tenant_id, namespace_id);

    let define_output = define_and_start_run(
        &client,
        &spec,
        scenario_id.clone(),
        tenant_id,
...
async fn metamorphic_concurrent_triggers_deterministic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tenant_id = decision_gate_core::TenantId::from_raw(1)
        .ok_or_else(|| std::io::Error::other("nonzero tenant id"))?;
    let run_id = decision_gate_core::RunId::new("run-1");
    let spec = concurrent_trigger_spec(scenario_id.clone(), tenant_id, namespace_id);
    ...
    let trigger_a = build_trigger_request(
        &define_output.scenario_id,
        &run_id,
        tenant_id,
        namespace_id,
    ...
    let trigger_b = build_trigger_request(
        &define_output.scenario_id,
        &run_id,
        tenant_id,
        namespace_id,
    ...
    if result_a.decision.trigger_id.as_str() != "trigger-a" {
        let trigger_id = result_a.decision.trigger_id.as_str();
        return Err(format!("unexpected decision trigger id for trigger-a: {trigger_id}").into());
    }
    if result_b.decision.trigger_id.as_str() != "trigger-b" {
        let trigger_id = result_b.decision.trigger_id.as_str();
        return Err(format!("unexpected decision trigger id for trigger-b: {trigger_id}").into());
    }
    ...
    .await?;
    verify_trigger_decision_records(&runpack_dir)?;
