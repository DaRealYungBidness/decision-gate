...
async fn perf_core_mcp_throughput_release() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let workers = context.target.workers;
    let warmup_distribution = distribute_iterations(context.target.warmup_iterations, workers)?;
    let measure_distribution = distribute_iterations(context.target.measure_iterations, workers)?;
...
async fn perf_precheck_throughput_release() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let scenario_id = define_fixture_scenario(&context.client, fixture.clone()).await?;
    let schema_record = perf_schema_record(fixture.tenant_id, fixture.namespace_id, "perf-precheck", "v1");
...
async fn perf_registry_mixed_throughput_release() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for worker_idx in 0 .. workers {
        ...
        let measured = measure_distribution[worker_idx];
        joins.spawn(async move {
            run_registry_worker(&client, tenant_id, namespace_id, worker_idx, warmup, measured).await
        });
    }
    let samples = gather_worker_samples(&mut joins).await?;
    let elapsed = started.elapsed();
...
async fn init_perf_context(
    ...
    let target_file = load_perf_targets()?;
    let target = target_file
        .tests
        .get(test_key)
        .cloned()
        .ok_or_else(|| format!("missing performance target `{test_key}` in `{PERF_TARGETS_FILE}`"))?;
...
fn load_perf_targets() -> Result<PerfTargetsFile, Box<dyn std::error::Error>> {
    let path = perf_targets_path();
...
fn perf_targets_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(PERF_TARGETS_FILE)
}
...
async fn define_fixture_scenario(
    ...
    let define_input =
        serde_json::to_value(&define_request).map_err(|err| format!("serialize scenario_define: {err}"))?;
...
async fn register_schema(client: &McpHttpClient, record: DataShapeRecord) -> Result<(), String> {
    ...
    let input =
        serde_json::to_value(&request).map_err(|err| format!("serialize schemas_register: {err}"))?;
    let _: Value = client
...
async fn run_core_worker(
    ...
    for local_idx in 0 .. total {
        let seq_local = u64::try_from(local_idx).map_err(|_| "worker iteration overflow".to_string())?;
        let global_seq = seq_local.saturating_mul(workers_u64).saturating_add(1);
        let run_id = decision_gate_core::RunId::new(format!("perf-core-{worker_idx:02}-{local_idx:05}"));
        ...
        let start_input =
            serde_json::to_value(&start_request).map_err(|err| format!("serialize scenario_start: {err}"))?;
        let start_measured = local_idx >= warmup;
        let _: decision_gate_core::RunState = timed_tool_call(
            client,
            "scenario_start",
            start_input,
            start_measured,
        ...
        let trigger_request = ScenarioTriggerRequest {
            ...
            trigger: decision_gate_core::TriggerEvent {
                ...
                tenant_id: fixture.tenant_id,
                namespace_id: fixture.namespace_id,
                trigger_id: TriggerId::new(format!("trigger-{worker_idx:02}-{local_idx:05}")),
        ...
        let trigger_input =
            serde_json::to_value(&trigger_request).map_err(|err| format!("serialize scenario_trigger: {err}"))?;
...
async fn run_precheck_worker(
    ...
    for local_idx in 0 .. total {
        ...
        let input = serde_json::to_value(&request).map_err(|err| format!("serialize precheck: {err}"))?;
...
async fn run_registry_worker(
    ...
    for local_idx in 0 .. total {
        let record = perf_schema_record(
            tenant_id,
            namespace_id,
            &format!("perf-registry-{worker_idx:02}-{local_idx:05}"),
            "v1",
        );
        ...
        let list_input =
            serde_json::to_value(&list_request).map_err(|err| format!("serialize schemas_list: {err}"))?;
...
fn percentile_ms(latencies: &[u64], percentile: u32) -> Result<u64, String> {
    ...
    let rank = len_u128
        .saturating_mul(percentile_u128)
        .saturating_add(99)
        .saturating_div(100)
        .max(1);
    ...
    sorted
        .get(index)
        .copied()
        .ok_or_else(|| "percentile index out of bounds".to_string())
}

fn summarize_samples(samples: &[CallSample], elapsed: Duration) -> Result<AggregateMetrics, String> {
    ...
    let total_duration_ms = samples
        .iter()
        .fold(0u64, |acc, sample| acc.saturating_add(sample.duration_ms));
    ...
    let total_calls_u64 = u64::try_from(total_calls).map_err(|_| "total calls overflow".to_string())?;
    let failed_calls_u64 = u64::try_from(failed_calls).map_err(|_| "failed calls overflow".to_string())?;
    let total_calls_u32 =
        u32::try_from(total_calls_u64).map_err(|_| "total calls too large for f64 conversion".to_string())?;
    let failed_calls_u32 =
        u32::try_from(failed_calls_u64).map_err(|_| "failed calls too large for f64 conversion".to_string())?;
...
fn build_tool_metrics(
    ...
    for (tool, entries) in by_tool {
        ...
        let calls_u32 =
            u32::try_from(calls_u64).map_err(|_| "tool calls too large for f64 conversion".to_string())?;
...
fn finalize_perf_report(
    ...
    let slo_enforced = should_enforce_slo();
    let violations = if slo_enforced {
        evaluate_slo(&metrics, &context.target)
    } else {
        Vec::new()
    };
    ...
    context
        .reporter
        .artifacts()
        .write_json("perf_summary.json", &summary)?;
    context
        .reporter
        .artifacts()
        .write_json("perf_tool_metrics.json", &tool_latency_ranking)?;
    context
        .reporter
        .artifacts()
        .write_json("tool_transcript.json", &transcript)?;
    ...
    if !slo_enforced {
        notes.push(format!(
            "SLO assertions skipped via {}",
            PERF_SKIP_SLO_ASSERTS_ENV
        ));
