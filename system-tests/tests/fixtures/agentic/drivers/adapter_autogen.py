#!/usr/bin/env python3
# system-tests/tests/fixtures/agentic/drivers/adapter_autogen.py
# ============================================================================
# Module: Agentic Flow Harness AutoGen Driver
# Description: Executes a scenario pack via the Decision Gate AutoGen tools.
# ============================================================================
...
try:
    ...
    print(json.dumps({"status": "skipped", "driver": "autogen", "reason": str(exc)}))
    sys.exit(0)
...
async def run_tool(tool, request: dict[str, Any]) -> dict[str, Any]:
    cancellation = CancellationToken()
    return await tool.run_json({"request": request}, cancellation)
...
def main() -> int:
    ...
    async def run_sequence() -> dict[str, Any]:
        await run_tool(tool_map["decision_gate_scenario_define"], {"spec": spec})
        await run_tool(
            tool_map["decision_gate_scenario_start"],
            {
        ...
        await run_tool(
            tool_map["decision_gate_scenario_trigger"],
            {"scenario_id": run_config["scenario_id"], "trigger": trigger},
        )
        status = await run_tool(
            tool_map["decision_gate_scenario_status"],
            {
                "scenario_id": run_config["scenario_id"],
                "request": {
        ...
        nonlocal runpack_dir
        if not runpack_dir:
            runpack_dir = tempfile.mkdtemp(prefix="dg-agentic-runpack-")

        export = await run_tool(
            tool_map["decision_gate_runpack_export"],
            {
                "scenario_id": run_config["scenario_id"],
                "run_id": run_config["run_id"],
                "tenant_id": run_config["tenant_id"],
        ...
        return {"status": status, "export": export}
