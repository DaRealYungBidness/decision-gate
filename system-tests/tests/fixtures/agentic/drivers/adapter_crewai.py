#!/usr/bin/env python3
# system-tests/tests/fixtures/agentic/drivers/adapter_crewai.py
# ============================================================================
# Module: Agentic Flow Harness CrewAI Driver
# Description: Executes a scenario pack via the Decision Gate CrewAI tools.
# ============================================================================
...
try:
    ...
    from decision_gate_crewai import (
    ...
    print(json.dumps({"status": "skipped", "driver": "crewai", "reason": str(exc)}))
    sys.exit(0)
...
def run_tool(tool, request: dict[str, Any]) -> dict[str, Any]:
    result_json = tool.run(request=request)
...
def main() -> int:
    ...
    define_tool = DecisionGateScenarioDefineTool(client=client)
    start_tool = DecisionGateScenarioStartTool(client=client)
    trigger_tool = DecisionGateScenarioTriggerTool(client=client)
    status_tool = DecisionGateScenarioStatusTool(client=client)
    export_tool = DecisionGateRunpackExportTool(client=client)

    run_tool(define_tool, {"spec": spec})
    run_tool(
        start_tool,
        {
    ...
    run_tool(trigger_tool, {"scenario_id": run_config["scenario_id"], "trigger": trigger})
