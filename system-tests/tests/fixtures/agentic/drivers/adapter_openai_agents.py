#!/usr/bin/env python3
# system-tests/tests/fixtures/agentic/drivers/adapter_openai_agents.py
# ============================================================================
# Module: Agentic Flow Harness OpenAI Agents Driver
# Description: Executes a scenario pack via the Decision Gate OpenAI Agents tools.
# ============================================================================
...
try:
    ...
    print(json.dumps({"status": "skipped", "driver": "openai_agents", "reason": str(exc)}))
    sys.exit(0)
...
def find_tool(tools: list[object], name: str) -> object:
    for tool in tools:
...
def invoke_tool(tool: object, request: dict[str, Any]) -> dict[str, Any]:
    # Best-effort invocation across OpenAI Agents versions.
    if hasattr(tool, "on_invoke_tool"):
        payload = json.dumps({"request": request})
        ctx = ToolContext(
            context=None,
            tool_name=getattr(tool, "name", "decision_gate_tool"),
            tool_call_id="dg-openai-agents",
            tool_arguments=payload,
        )
        result = tool.on_invoke_tool(ctx, payload)
    elif hasattr(tool, "invoke"):
        result = tool.invoke({"request": request})
        ...
        raise RuntimeError("openai_agents tool invocation unsupported")
...
def main() -> int:
    ...
    define_tool = find_tool(tools, "decision_gate_scenario_define")
    start_tool = find_tool(tools, "decision_gate_scenario_start")
    trigger_tool = find_tool(tools, "decision_gate_scenario_trigger")
    status_tool = find_tool(tools, "decision_gate_scenario_status")
    export_tool = find_tool(tools, "decision_gate_runpack_export")

    invoke_tool(define_tool, {"spec": spec})
    invoke_tool(
        start_tool,
        {
    ...
    invoke_tool(trigger_tool, {"scenario_id": run_config["scenario_id"], "trigger": trigger})
