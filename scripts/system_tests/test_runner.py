#!/usr/bin/env python3
# scripts/system_tests/test_runner.py
# ============================================================================
...
def select_tests(
    ...
    if quick_only:
        quick_categories = {k for k, v in registry.get("categories", {}).items() if v.get("quick")}
...
def run_test(
    ...
    if isolate_target_dir:
        ...
        target_root = (
            Path(tempfile.gettempdir()) / "decision-gate-targets" / sanitize_name(test.name)
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--isolate-target-dir", action="store_true", help="Isolate target dir per test"
    )
    ...
    base_root = (
        Path(args.run_root)
        if args.run_root
        else workspace_root / ".tmp" / "system-tests" / f"run_{timestamp}"
    )
