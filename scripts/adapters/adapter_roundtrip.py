#!/usr/bin/env python3
# scripts/adapters/adapter_roundtrip.py
# =============================================================================
...
class AutoGenTool(NamedTool, Protocol):
    async def run_json(
        self, input: Mapping[str, JsonMapping], cancellation_token: object
    ) -> JsonLike: ...
...
class ToolBuilder(Protocol[TTool]):
    def __call__(
        self, client: DecisionGateClient, *, validate: bool = False
    ) -> Sequence[TTool]: ...
...
def _autogen_runner(framework: str) -> int:
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        return _exercise_roundtrip(
            framework, tools, fixture, lambda tool, req: asyncio.run(call_tool(tool, req))
        )
...
def _openai_agents_runner(framework: str) -> int:
    ...
    tool_context_type = cast(
        type[ToolContextLike], _load_attr("agents.tool_context", "ToolContext")
    )
...
def _exercise_roundtrip(
    ...
    result = caller(
        tool("decision_gate_docs_search"), {"query": "evidence flow", "max_sections": 3}
    )
