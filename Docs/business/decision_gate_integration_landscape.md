...
| **Documentation**              | Architecture docs (auth, namespaces, trust anchors, comparators, providers, runpacks, scenario state, system tests), guides (getting started, provider development, protocol, predicates, integration patterns, security, evidence flow, RET logic), generated artifacts (tool schemas, provider contracts, tooltips, example scenarios) |
